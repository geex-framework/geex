...
export class UserLoginComponent implements OnInit, OnDestroy {
    ...
    try {
      ...
      if (queryParamMap.has("code") && isWechatCallback) {
        try {
          ...
          const result = await geex.authenticationWechat.resolveLogin({
          ...
          if (result.isLinked && result.session?.token) {
            ...
            geex.authenticationWechat.establishSession({
          ...
          await this.router.navigate(["/authentication/user-login-link"], {
            queryParams: {
  ...
  async renderWechatQr(): Promise<void> {
    try {
      await geex.authenticationWechat.renderQr({
