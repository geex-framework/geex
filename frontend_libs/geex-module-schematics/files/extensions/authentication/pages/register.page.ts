...
import { CaptchaProvider } from "@geexcode/geex-extensions-captcha";
import { SharedModule } from "@/shared/shared.module";
import { registerAndSignIn } from "../graphql/operations.gql";
...
export class UserRegisterComponent implements OnDestroy {
  ...
  async getCaptcha(): Promise<void> {
    ...
    this.count = 59;
    const captcha = await geex.captcha.generateCaptcha({
      captchaProvider: CaptchaProvider.Sms,
      smsCaptchaPhoneNumber: this.mobile.value,
    });
  ...
  async submit(): Promise<void> {
    ...
    const valid = await geex.captcha.validateCaptcha({
      captchaProvider: CaptchaProvider.Sms,
