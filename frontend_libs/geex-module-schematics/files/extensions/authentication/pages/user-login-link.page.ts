...
export class UserLoginLinkPage implements OnInit {
  ...
  async linkExisting(): Promise<void> {
    ...
    try {
      ...
      const session = await geex.authenticationWechat.linkLogin(this.userLoginLinkToken, {
        headers: {
      ...
      geex.authenticationWechat.establishSession({ token: session.token, returnUrl: "/" });
    } catch (e: any) {
  ...
  async registerAndLink(): Promise<void> {
    ...
    this.error = "";
    try {
      const session = await geex.authenticationWechat.registerAndLinkLogin({
