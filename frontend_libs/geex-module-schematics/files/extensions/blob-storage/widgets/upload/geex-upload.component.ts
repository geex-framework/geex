...
export class GeexUploadComponent implements ControlValueAccessor {
  ...
  private readonly createDocument = inject(GEEX_BLOB_CREATE_DOCUMENT, { optional: true });
  private readonly listDocument = inject(GEEX_BLOB_LIST_DOCUMENT, { optional: true });
  private readonly deleteDocument = inject(GEEX_BLOB_DELETE_DOCUMENT, { optional: true });
  private readonly defaultStorageType = inject(GEEX_BLOB_DEFAULT_STORAGE_TYPE);
  ...
  handleRemove = (file: NzUploadFile): boolean | Observable<boolean> => {
    ...
    if (this.deleteRemoteFile) {
      return this.deleteRemoteFile.pipe(
        switchMap(isDelete => {
          if (isDelete && this.deleteDocument) {
            const ids = [file.uid];
            return this.apollo
              .mutate({
                ...
                variables: {
                  request: {
                    ids,
                    storageType: this.storageType ?? this.defaultStorageType,
                  },
                },
              })
              .firstValuePromise()
  ...
  private async fetchFilesByIds(ids: string[] | string): Promise<NzUploadFile[]> {
    ...
    const blobs = (result?.data as { blobObjects: { items: Array<Record<string, unknown>> } } | undefined)?.blobObjects;
    if (!blobs) {
      return [];
    }
