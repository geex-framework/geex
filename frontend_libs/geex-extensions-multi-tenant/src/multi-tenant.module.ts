...
import type { Tenant, MultiTenantModule } from "./multi-tenant.types";

const GQL_CHECK_TENANT = gql`mutation checkTenant($code: String!) { checkTenant(code: $code) { id code name isEnabled createdOn } }`;

export function createMultiTenantModule(injector: Injector): MultiTenantModule {
  ...
  return module as MultiTenantModule;
}
