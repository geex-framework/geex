...
echo "building gql proxy with config '$cwd/../../x_Org_x.x_Proj_x.Server/appsettings.$env.json'"
# 删除注释
$configJson = (get-content -Path "$cwd/../../x_Org_x.x_Proj_x.Server/appsettings.$env.json") -replace '^\s*\/\/.+$', '';
# Json.Net兼容尾逗号
$config = ([System.Text.Json.JsonDocument]::Parse($configJson, $JsonDocumentOptions)).RootElement.ToString() | ConvertFrom-Json
# $config = (get-content -Path "$cwd/../../x_Org_x.x_Proj_x.Server/appsettings.$env.json") | ConvertFrom-Json
