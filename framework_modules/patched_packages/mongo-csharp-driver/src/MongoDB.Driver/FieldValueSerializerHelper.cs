...
namespace MongoDB.Driver
{
    internal static class FieldValueSerializerHelper
    {
        ...
        public static IBsonSerializer GetSerializerForValueType(IBsonSerializer fieldSerializer, IBsonSerializerRegistry serializerRegistry, Type valueType, bool allowScalarValueForArrayField)
        {
            ...
            bool isNullable = ReflectionExtensions.IsNullable(valueType);
            Type nullableUnderlyingType = valueType.GetNullableUnderlyingType();
            if (isNullable && nullableUnderlyingType == fieldType)
            {
            ...
            if (fieldTypeInfo.IsEnum)
            {
                ...
                if (isNullable && nullableUnderlyingType.IsConvertibleToEnum())
                {
                    var underlyingValueType = nullableUnderlyingType;
            ...
            if (ReflectionExtensions.IsNullableEnum(fieldType) && isNullable)
            {
                var nonNullableFieldType = fieldType.GetNullableUnderlyingType();
                var nonNullableValueType = nullableUnderlyingType;
