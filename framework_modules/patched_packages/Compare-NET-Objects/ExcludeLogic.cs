...
namespace KellermanSoftware.CompareNetObjects
{
    ...
    public static class ExcludeLogic
    {
        ...
        public static bool ShouldExcludeMember(ComparisonConfig config, MemberInfo info, Type objectType)
        {
            ...
            if (IgnoredByReadOnly(config, info))
                return true;

            if (IgnoredByLogicGetters(config, info))
                return true;
        ...
        private static Boolean IgnoredByReadOnly(ComparisonConfig config, MemberInfo info)
        {
            //Prevent loading attributes when AttributesToIgnore is empty
            if (!config.CompareReadOnly && info is PropertyInfo propertyInfo && propertyInfo.CanWrite == false)
            {
                return true;
            }
            return false;
        ...
        private static Boolean IgnoredByLogicGetters(ComparisonConfig config, MemberInfo info)
        {
            //Prevent loading attributes when AttributesToIgnore is empty
            if (config.IgnoreLogicGetters && info is PropertyInfo propertyInfo && propertyInfo.GetGetMethod()?.IsSpecialName != true)
            {
                return true;
            }
            return false;
