...
#region cert
$cert = Get-CCertificate -FriendlyName "dev.geex.com" -StoreLocation LocalMachine -StoreName Root
$pwd = ConvertTo-SecureString -String "dev.geex.com" -AsPlainText -Force
if ($cert -eq $null)
{
    ...
    $cert = New-SelfSignedCertificate -CertStoreLocation Cert:\LocalMachine\My -Subject dev.geex.com -DnsName dev.geex.com, *.api.dev.geex.com, *.dev.geex.com -FriendlyName "dev.geex.com" -NotAfter (Get-Date).AddYears(1000)
    mkdir ./.dev_cert
    Export-PfxCertificate -Cert "Cert:\LocalMachine\My\$($cert.Thumbprint)" -FilePath "./.dev_cert/dev.geex.com.pfx" -Password $pwd
    openssl pkcs12 -in "./.dev_cert/dev.geex.com.pfx" -nodes -out ./.dev_cert/dev.geex.com.pem -passin pass:dev.geex.com
    Install-CCertificate -Path "./.dev_cert/dev.geex.com.pfx" -StoreLocation LocalMachine -StoreName Root -Password $pwd
    cd ./dev_env
...
if (-not (Test-Path ".\\client\\bms\\Bms.Client\\BmsClientApp\\node_modules")) {
    $curDir = pwd;
    cd ".\\client\\bms\\Bms.Client\\BmsClientApp"
    yarn
    cd $curDir;
}

if (-not (Test-Path ".\\client\\bms\\Bms.Client\\BmsClientApp\\geex-schematics\\node_modules")) {
    $curDir = pwd;
    cd ".\\client\\bms\\Bms.Client\\BmsClientApp\\geex-schematics"
    yarn
    yarn build
    cd $curDir;
}
...
redbird({
    ...
    resolvers:[
      function(host, url, req) {
        if(host == 'bms.dev.geex.com'){
          return 'http://127.0.0.1:4201'
        }
        if(host == 'bms.api.dev.geex.com'){
          return 'https://127.0.0.1:8020'
        }
      }
    ],
    ssl: {
        key: './.dev_cert/dev.geex.com.pem',
        cert: './.dev_cert/dev.geex.com.pem',
...
# openssl pkcs12 -in "./.dev_cert/dev.geex.com.pfx" -nodes -out dev.geex.com.pem
# # (2)从.pem文件中导出私钥server.key：
# openssl rsa -in dev.geex.com.pem -out "./.dev_cert/dev.geex.com.key"
# #(3)从.pem文件中导出证书server.crt
# openssl x509 -in dev.geex.com.pem -out "./.dev_cert/dev.geex.com.crt"
