...
export function insertImport(source: ts.SourceFile, fileToEdit: string, symbolName: string, fileName: string, isDefault = false): Change {
...
export function getMetadataField(node: ts.ObjectLiteralExpression, metadataField: string): ts.ObjectLiteralElement[] {
  return (
    node.properties
      ...
      .filter(({ name }) => {
        return (ts.isIdentifier(name as ts.Node) || ts.isStringLiteral(name as ts.Node)) && name?.getText() === metadataField;
      })
  );
}
