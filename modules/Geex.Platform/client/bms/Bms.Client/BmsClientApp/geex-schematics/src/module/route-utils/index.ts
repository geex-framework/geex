...
export function addRouteToNgModule(options: PageOptions): Rule {
  ...
  return host => {
    ...
    const pagePath = `/${options.path}/${`${strings.dasherize(options.name)}/`}${strings.dasherize(options.name)}.module`;
    ...
    const changes = addRouteToRoutesArray(source, module, routePath, relativePath, ngModuleName, options.addToLayout);
  ...
  for (const keyword of keywords) {
    if (ts.isVariableStatement(keyword)) {
      ...
      if (ts.isVariableDeclaration(declaration) && declaration.initializer && declaration.name.getText() === "routes") {
        ...
        if (addToLayout?.length) {
          try {
            node = node.getChildren()!.find(x => x.getFullText().indexOf(addToLayout))!;
            // console.warn(1 + node.getFullText());
            node = node.getChildren()!.find(x => x.getFullText().indexOf(addToLayout) > 0)!;
...
export function findRoutesFile(host: Tree, options: PageOptions): Path | null {
  const pathToCheck = `${options.path || ""}/${strings.dasherize(options.name)}`;
...
export function addRoute(options: PageOptions): Rule {
  return host => {
    ...
    const pagePath = `/${options.path}/${`${strings.dasherize(options.name)}/`}${strings.dasherize(options.name)}.page`;

    const relativePath = buildRelativePath(routesFile, pagePath);

    const routePath = strings.dasherize(options.routePath ? options.routePath : options.name);
    const componentImport = `${strings.classify(options.name)}ListPage`;
